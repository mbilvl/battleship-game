  ...
  if (valid) {
    if (orient == 0) {
      for (i = y; i < y + req; i++) {
        ...
        td.style.border = "2px solid #1e3a8a";
      }
...
function directionHandler(x, y) {
  if (x >= 0 && x <= 9 && y >= 0 && y <= 9) {
    //make sure the square is within the boundaries of grid
...
function whoWins() {
  ...
  if (player1.hits == 17) {
    ...
    msg.innerHTML = "🎉 Victory! You have won the battle! 🎉";
    msg.className =
      "text-center text-2xl font-bold mb-6 min-h-[2rem] animate-pulse";
  } else if (bot.hits == 17) {
    msg.style.color = "#dc2626";
    msg.innerHTML = "💥 Defeat! You have lost the battle! 💥";
    msg.className =
      "text-center text-2xl font-bold mb-6 min-h-[2rem] animate-pulse";
  }
}
...
function setOrientation(arg) {
  ...
  if (arg == 0) {
    ...
    o.className = "text-sm font-medium text-green-600";
    toggleSlider.className =
      "absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-200 ease-in-out transform translate-x-0";
  } else if (arg == 1) {
    orient = 1; //vertical
    o.innerHTML = "Vertical";
    o.className = "text-sm font-medium text-blue-600";
    toggleSlider.className =
      "absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform duration-200 ease-in-out transform translate-x-6";
  }
}

//toggle orientation function for the new UI
...
function successShot(shooter, victim, squareId, hitId, x, y) {
  ...
  var span = document.getElementById(squareId);
...
function shipPlacementMessage() {
  ...
  var shipName =
    player1.ship[player1.placed].name.charAt(0).toUpperCase() +
    player1.ship[player1.placed].name.slice(1);
  var shipMessage = "Place " + shipName + " (" + msgReq + " spaces)";
